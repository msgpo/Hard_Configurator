...
; Modified ......: BB_19 - Changed the return parameter $array[2] to return the width of the line with the biggest width instead of the provided $iMaxWidth.
...
Func _StringSize($sText, $iSize = 8.5, $iWeight = 400, $iAttrib = 0, $sName = "", $iMaxWidth = 0, $hWnd = 0)
	...
	Local $iMaxLineWidth;Set variable for the line with the biggest width
			...
			Else
				...
				While 1
					...
					If $i > StringLen($asLines[$j]) Then
						...
						If $iLine_Width > $iMaxLineWidth Then $iMaxLineWidth = $iLine_Width;Check if current line width exceedes the saved max line width
					...
					Else
						...
						DllCall("gdi32.dll", "bool", "GetTextExtentPoint32W", "handle", $hDC, "wstr", StringLeft($sTest_Line, $iLast_Word), "int", StringLen(StringLeft($sTest_Line, $iLast_Word)), "ptr", DllStructGetPtr($tSize))
						If @error Then Return SetError(2, _StringSize_Error_Close(6, $hDC, $hFont, $hLabel), 0)
						If DllStructGetData($tSize, "X") > $iMaxLineWidth Then;Check if current line width exceedes the saved max line width
							$iMaxLineWidth = DllStructGetData($tSize, "X")
						EndIf
