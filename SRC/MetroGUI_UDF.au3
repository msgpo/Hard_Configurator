;> Fixed a bug which caused the script to hang up/crash while exiting with multiple MetroGUIs open.
;> Fixed a bug which caused the custom set control cursor to be overwritten.
;> Fixed a bug which caused the automated handling of fullscreen toggle to malfunction in certain situations.
;> Optimized the text wrapping for _Metro_MsgBox
;> Added _Metro_LoadingAnimCreate to create a big loading animation hovering over the gui.
;> Integrated the _GUIDisable from guinness and modified it to work with the loading-animation.
...
; StringSize.au3 is in Hard_Configurator folder
...
Global $oMetroAnimation

;Global Variables for _GUIDisable by Guinness
Global $__aGUIDisable[3]
...
;=============================================MsgBox===============================================
;_Metro_MsgBox - Creates a MsgBox with a OK button and displays the text. _GUIDisable($GUI,  30) should be used before, so the MsgBox is better visible and afterwards _GUIDisable($GUI).
...
Func _Metro_FullscreenToggle($mGUI)
	...
	If $iGUI_LIST[$iGui_Count][11] Then ;Already in fullscreen -> Restore
		...
		Else
			...
			EndIf
		EndIf
		GUICtrlSetState($FullscreenRsBtn, 32 + 128)
		GUICtrlSetState($FullScreenBtn, 16 + 64)
	...
	Else ;Not in fullscreen mode -> Enter fullscreen mode
		...
		GUICtrlSetState($FullScreenBtn, 32 + 128)
		If $MaxBtn Then
			GUICtrlSetState($MaxBtn, 32)
			GUICtrlSetState($RestoreBtn, 32)
		EndIf
		GUICtrlSetState($FullscreenRsBtn, 16 + 64)
...
Func _iCreateMButton($Text, $Left, $Top, $Width, $Height, $BG_Color = $GUIThemeColor, $Font_Color = $FontThemeColor, $Font = "Arial", $Fontsize = 9, $FontStyle = 1)
	...
	If StringInStr($GUI_Theme_Name, "Light") Then
		Local $BG_ColorD = _RGBtoARGB(_AlterBrightness($GUIThemeColor, -16))
		$BG_Color = _RGBtoARGB(_AlterBrightness($GUIThemeColor, -30))
	Else
		Local $BG_ColorD = _RGBtoARGB(_AlterBrightness($GUIThemeColor, +12))
		$BG_Color = _RGBtoARGB(_AlterBrightness($GUIThemeColor, +25))
	EndIf
...
Func _Metro_RightClickMenu($mGUI, $mWidth, $ButtonsArray, $bFont = "Segoe UI", $bFontSize = 10, $bFontStyle = 0, $mTitle = "")
	Local $mPos = MouseGetPos()
	Local $iButtonsArray[UBound($ButtonsArray)]
	Local $ButtonStep = cDPI(28)
	Local $bMargin = cDPI(1)
	Local $mLableMargin = 0
	Local $mLableMarginLeft = 0
	Local $mLabelHeight = 0
	...
	If $mTitle <> "" Then
		$mLableMargin = cDPI(4)
		$mLabelHeight = cDPI(20)
		$mLableMarginLeft = cDPI(5)
	EndIf

	Local $btnPos = _CalcMenuBtnsPos($bMargin, $bMargin + ($mLableMargin * 2) + $mLabelHeight, $mWidth - ($bMargin * 2), $ButtonStep, $bMargin, UBound($ButtonsArray))

	;Fix position if  it is offscreen
	Local $mHeight = $btnPos[UBound($btnPos) - 1][0]
	...
	GUISetBkColor($GUIThemeColor)
	_CreateBorder($mGUI, $mWidth, $mHeight)

	If $mTitle <> "" Then
		GUICtrlCreateLabel($mTitle, $mLableMarginLeft, $mLableMargin, $mWidth - ($mLableMarginLeft * 2), $mLabelHeight, 0x1)
		GUICtrlSetFont(-1, 10, 600, 0, "Segoe UI", 5)
		GUICtrlSetColor(-1, $FontThemeColor)
		GUICtrlSetResizing(-1, 768 + 8)
	EndIf


	For $iB = 0 To UBound($ButtonsArray) - 1 Step +1
		$iButtonsArray[$iB] = _iCreateMButton($ButtonsArray[$iB], $btnPos[$iB][0], $btnPos[$iB][1], $btnPos[$iB][2], $btnPos[$iB][3], $GUIThemeColor, $FontThemeColor, $bFont, $bFontSize, $bFontStyle)
	Next
	GUISetState(@SW_SHOW, $MenuForm)
...
Func _CalcMenuBtnsPos($bStartPosX, $bStartPosY, $bWidth, $bHeight, $MarginBetween, $bCount)
	$bStartPosX = Floor($bStartPosX)
	$bStartPosY = Floor($bStartPosY)
	$bWidth = Floor($bWidth)
	$bHeight = Floor($bHeight)
	$MarginBetween = Floor($MarginBetween)

	Local $mButtonsArray[$bCount + 1][4]
	For $i = 0 To $bCount - 1
		$mButtonsArray[$i][0] = $bStartPosX
		$mButtonsArray[$i][1] = $bStartPosY + ($i * $bHeight) + ($i * $MarginBetween)
		$mButtonsArray[$i][2] = $bWidth
		$mButtonsArray[$i][3] = $bHeight
	Next
	$mButtonsArray[$bCount][0] = $mButtonsArray[$bCount - 1][1] + $MarginBetween + $bHeight ;Total Height

	Return $mButtonsArray
EndFunc   ;==>_CalcMenuBtnsPos
...
Func _Metro_CreateButtonEx2($Text, $Left, $Top, $Width, $Height, $BG_Color = $ButtonBKColor, $Font_Color = $ButtonTextColor, $Font = "Calibri", $Fontsize = 11, $FontStyle = 0)
	Local $Button_Array[16]
	...
	;Create font, Set font options
	Local $hFormat = _GDIPlus_StringFormatCreate(), $hFamily = _GDIPlus_FontFamilyCreate($Font), $hFont = _GDIPlus_FontCreate($hFamily, $Fontsize, $FontStyle)
	Local $tLayout = _GDIPlus_RectFCreate(0, 1 * $btnDPI, $Width, $Height - (1 * $btnDPI))
	_GDIPlus_StringFormatSetAlign($hFormat, 1)
	_GDIPlus_StringFormatSetLineAlign($hFormat, 1)
...
;
; Notes .......: _GUIDisable($GUI, 30) should be used before starting the MsgBox, so the MsgBox is better visible on top of your GUI. You also have to call _GUIDisable($GUI) afterwards.
; ===============================================================================================================================
Func _Metro_MsgBox($Flag, $Title, $Text, $mWidth = 600, $Fontsize = 11, $ParentGUI = "", $Timeout = 0)
	...
	Switch $Buttons_Count
		Case 1
			$iMinWidth = 120 * $msgbDPI
		Case 2
			$iMinWidth = 240 * $msgbDPI
		Case 3
			$iMinWidth = 360 * $msgbDPI
	EndSwitch
	...
	If ($mWidth < $iMinWidth) Then
		ConsoleWrite("MetroUDF-Error: Messagebox width has to be at least " & $iMinWidth / $msgbDPI & " for the selected message style/flag." & @CRLF)
		$mWidth = $iMinWidth
	EndIf
	...
	Local $iTotalWidth = $LabelSize[2] + (30 * $msgbDPI)


	;Update total width according to the text size.
	If $mWidth > $iTotalWidth Then
		If $iTotalWidth < $iMinWidth Then
			$mWidth = $iMinWidth
		Else
			$mWidth = $iTotalWidth
		EndIf
	...
	Local $lOnEventMode = _CheckOnEventMode()
	If $lOnEventMode Then Opt("GUIOnEventMode", 0) ;Temporarily deactivate oneventmode
	...
	While 1
		If $Timeout <> 0 Then
			If $iMsgBoxTimeout <= 0 Then
				...
				If $lOnEventMode Then Opt("GUIOnEventMode", 1) ;Reactivate oneventmode
				Return SetError(1)
			EndIf
		EndIf
...
Func _CreateBorder($mGUI, $guiW, $guiH, $bordercolor = $GUIBorderColor)
	...
	Switch $imsg
		...
		Case 0x0084 ;Set mouse cursor for resizing etc. / Allow the upper GUI (28 pixel from top) to act as a control bar (doubleclick to maximize, move gui around..)
			If $iGUI_LIST[$gID][2] And Not $iGUI_LIST[$gID][11] Then ;If resize is allowed and not in fullscreen mode
				...
				If Abs(BitAND(BitShift($lParam, 16), 0xFFFF) - $wPos[1]) < (30 * $gDPI) Then Return $HTCAPTION
			EndIf
		...
		Case 0x0020 ;Reset cursor back to defaul to prevent from resize cursors getting stuck
			Local $cc = MouseGetCursor()
			If $cc > 7 And $cc < 14 Then
				Local $curInf = GUIGetCursorInfo($hWnd)
...
; Syntax ........: _GUIDisable($hWnd[, $iBrightness = 50[, $bColor = 0x000000]])
; Parameters ....: $hWnd                	- Handle to the GUI to be disabled.
;                  			  $iBrightness     	- [optional] Brightness 0-100. Default is 50
;                  			  $bColor              	- [optional] Background color. Default is $GUIThemeColor
;                  			  $iAnimation        - [optional] Create metro loading animation. Default is false
; Return values .: None
; Author ........: guinness
...
; Modified ......: BB_19
; ===============================================================================================================================
Func _GUIDisable($hWnd, $iBrightness = 40, $bColor = $GUIThemeColor, $iAnimation = False)
;~ 	Return
	If IsHWnd($__aGUIDisable[0]) Then
		_Metro_LoadingAnimDestroy()
		GUIDelete($__aGUIDisable[0])
		GUISwitch($__aGUIDisable[1])
		$__aGUIDisable[0] = 0
		$__aGUIDisable[1] = 0
	Else
		$__aGUIDisable[1] = $hWnd
		Local $iStyle = GUIGetStyle($hWnd)
		Local $aWinGetPos = WinGetPos($hWnd)
		$__aGUIDisable[0] = GUICreate('', $aWinGetPos[2], $aWinGetPos[3], $aWinGetPos[0], $aWinGetPos[1], $WS_POPUP, -1, $hWnd)
		GUISetBkColor($bColor, $__aGUIDisable[0])
		WinSetTrans($__aGUIDisable[0], '', Round($iBrightness * (255 / 100)))

		If $iAnimation Then
			_Metro_LoadingAnimCreate($hWnd)
		EndIf
		GUISetState(@SW_SHOWNOACTIVATE, $__aGUIDisable[0])
		GUISetState(@SW_DISABLE, $__aGUIDisable[0])
		GUISwitch($hWnd)
	EndIf
	Return $__aGUIDisable[0]
EndFunc   ;==>_GUIDisable



; #FUNCTION# ====================================================================================================================
; Name ..........: _Metro_LoadingAnimCreate
; Description ...: Creates a Loading animation on the provided GUI.  - Only one active instance supported!
; Syntax ........: _Metro_LoadingAnimCreate($hGUI[, $iX = Default[, $iY = Default[, $iSize = Default[, $iPenSize = Default[,
;                  $iStep = 3[, $iCircleColor = $ButtonBKColor]]]]]])
; Parameters ....: $hGUI                - a handle value.
;                  $iX                  - [optional] x Position. Default is ""
;                  $iY                  - [optional] y Position. Default is ""
;                  $iSize               - [optional] Size of the circle. Default is ""
;                  $iPenSize            - [optional] Size of the circle border / Pen size. Default is ""
;                  $iStep               - [optional] Steps per loop, the higher the faster the circle spins. Default is 3.
;                  $iCircleColor        - [optional] RGB color of the circle. Default is $ButtonBKColor.
; Author ........: BB_19
; Note ............: You don't have to provide any size or position parameters if you want to create a big loading anmation in the middle of the GUI, the function will calculate the parameters.
; Note ............: You have to destroy the animation to release the resources after you are finished with it, using: _Metro_LoadingAnimDestroy()
; ===============================================================================================================================
Func _Metro_LoadingAnimCreate($hGUI, $iX = "", $iY = "", $iSize = "", $iPenSize = "", $iStep = 3, $iCircleColor = $ButtonBKColor)
	If IsDllStruct($oMetroAnimation) Then _Metro_LoadingAnimDestroy() ;Delete previous instance first if exists
	Local $aWinGetPos = WinGetPos($hGUI)
	;Get Circle Size
	If $iSize == "" Then
		If $aWinGetPos[2] < $aWinGetPos[3] Then
			$iSize = $aWinGetPos[2]
		Else
			$iSize = $aWinGetPos[3]
		EndIf
		$iSize = Floor($iSize * 0.4)
	Else
		If $HIGHDPI_SUPPORT Then $iSize = Floor($iSize * $gDPI)
	EndIf
	;Get PenSize / Circle Border Size
	If $iPenSize == "" Then
		$iPenSize = $iSize / 20
	Else
		If $HIGHDPI_SUPPORT Then $iPenSize = Floor($iPenSize * $gDPI)
	EndIf
	If $iPenSize = 0 Then $iPenSize = 1
	;Get Circle Position
	If $iX == "" Then
		$iX = $aWinGetPos[0] + ($aWinGetPos[2] - $iSize) / 2
	Else
		If $HIGHDPI_SUPPORT Then $iX = Floor($iX * $gDPI)
	EndIf
	If $iY == "" Then
		$iY = $aWinGetPos[1] + ($aWinGetPos[3] - $iSize) / 2
	Else
		If $HIGHDPI_SUPPORT Then $iY = Floor($iY * $gDPI)
	EndIf
	$oMetroAnimation = DllStructCreate("handle ParentGUI;handle GUI;handle Bitmap;handle Graphics;handle Pen;int Step;int CurrentStep;double Size;int PenSize;bool LoopActive")
	$oMetroAnimation.ParentGUI = $hGUI
	$oMetroAnimation.GUI = GUICreate("", $iSize, $iSize, $iX, $iY, $WS_POPUP, $WS_EX_LAYERED, $oMetroAnimation.ParentGUI)
;~ 	_WinAPI_SetParent($oMetroAnimation.GUI, $oMetroAnimation.ParentGUI)
	$oMetroAnimation.Bitmap = _GDIPlus_BitmapCreateFromScan0($iSize, $iSize)
	$oMetroAnimation.Graphics = _GDIPlus_ImageGetGraphicsContext($oMetroAnimation.Bitmap)
	_GDIPlus_GraphicsSetSmoothingMode($oMetroAnimation.Graphics, 2)
	$oMetroAnimation.Pen = _GDIPlus_PenCreate(_RGBtoARGB($iCircleColor), $iPenSize)
	_GDIPlus_PenSetEndCap($oMetroAnimation.Pen, $GDIP_LINECAPROUND)
	_GDIPlus_PenSetStartCap($oMetroAnimation.Pen, $GDIP_LINECAPROUND)
	$oMetroAnimation.Step = $iStep
	$oMetroAnimation.CurrentStep = 0
	$oMetroAnimation.Size = $iSize
	$oMetroAnimation.PenSize = $iPenSize
	$oMetroAnimation.LoopActive = False
	GUISetState(@SW_SHOWNOACTIVATE, $oMetroAnimation.GUI)
;~ 	GUISetState(@SW_DISABLE, $oMetroAnimation.GUI)
	AdlibRegister("_iMetro_LoadingAnimLoop", 5)
EndFunc   ;==>_Metro_LoadingAnimCreate

Func _Metro_LoadingAnimDestroy()
	AdlibUnRegister("_iMetro_LoadingAnimLoop")
	If Not IsDllStruct($oMetroAnimation) Then Return SetError(1, 0, "Error: Animation already deleted?")
	GUIRegisterMsg($WM_TIMER, "")
	_GDIPlus_PenDispose($oMetroAnimation.Pen)
	_GDIPlus_GraphicsDispose($oMetroAnimation.Graphics)
	_GDIPlus_BitmapDispose($oMetroAnimation.Bitmap)
	GUIDelete($oMetroAnimation.GUI)
	GUISwitch($oMetroAnimation.ParentGUI)
	$oMetroAnimation = ""
EndFunc   ;==>_Metro_LoadingAnimDestroy


Func _iMetro_LoadingAnimLoop()
	If Not IsDllStruct($oMetroAnimation) Then Return
	If $oMetroAnimation.LoopActive Then Return
	$oMetroAnimation.LoopActive = True
	$oMetroAnimation.CurrentStep += $oMetroAnimation.Step
	If $oMetroAnimation.CurrentStep >= 360 Then $oMetroAnimation.CurrentStep = -360
	_GDIPlus_GraphicsClear($oMetroAnimation.Graphics, $GUIThemeColor)
	_GDIPlus_GraphicsDrawArc($oMetroAnimation.Graphics, $oMetroAnimation.PenSize, $oMetroAnimation.PenSize, $oMetroAnimation.Size - ($oMetroAnimation.PenSize * 2), $oMetroAnimation.Size - ($oMetroAnimation.PenSize * 2), $oMetroAnimation.CurrentStep, $oMetroAnimation.CurrentStep, $oMetroAnimation.Pen)
	Local $hHBitmap = _GDIPlus_BitmapCreateHBITMAPFromBitmap($oMetroAnimation.Bitmap)
	_WinAPI_BitmapDisplayTransparentInGUI($hHBitmap, $oMetroAnimation.GUI)
	_WinAPI_DeleteObject($hHBitmap)
	$oMetroAnimation.LoopActive = False
EndFunc   ;==>_iMetro_LoadingAnimLoop


; #FUNCTION# ====================================================================================================================
; Name ..........: _WinAPI_BitmapDisplayTransparentInGUI
; Author ........: UEZ
; ===============================================================================================================================
Func _WinAPI_BitmapDisplayTransparentInGUI(ByRef $hHBitmap, $hGUI)
...
Func _RGBtoARGB($iColor)
	Return $iColor + 0xFF000000
EndFunc   ;==>_RGBtoARGB

Func cDPI($iVal)
	If $HIGHDPI_SUPPORT Then
		Return Floor($iVal * $gDPI)
	Else
		Return $iVal
	EndIf
EndFunc   ;==>cDPI
