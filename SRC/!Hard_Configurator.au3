...
#AutoIt3Wrapper_Res_LegalCopyright=Copyright *  Andy Ful , June 2018
...
Global $BtnDocumentsAntiExploit
...
Global $Hard_ConfiguratorVersion
Global $ARlistview
Global $ARlistGUI
...
;Create Main GUI Buttons
  $Hard_ConfiguratorVersion = '4.0.0.0'
...
;  GUICtrlSetBkColor(-1, 0xdddddd)
  ...
  $BtnGeneralHelp = GUICtrlCreateButton("General Help", 270+$DeltaX, $DeltaY+30, 90, 25)
  GUICtrlSetOnEvent($BtnGeneralHelp,"Help0")
;  GUICtrlSetColor($BtnGeneralHelp, 0x000000)
;  GUICtrlSetBkColor(-1, 0xdddddd)

  $BtnTurnOnAllRestrictions = GUICtrlCreateButton("Recommended Restrictions", 370+$DeltaX-3, $DeltaY+30-3, 146, 25+6, -1, 1)
...
;  GUICtrlSetOnEvent(-1, "Help8")
  ...
  $BtnNoRemovableDisksExecution = GUICtrlCreateButton("No Removable Disks Exec.", 370+$DeltaX, $DeltaY+90, 140, 19)
  GUICtrlSetOnEvent(-1, "NoRemovableDisksExecution")
  $BtnHelpNoRemovableDisksExecution = GUICtrlCreateButton("Help", 320+$DeltaX, $DeltaY+90, 40, 19)
  GUICtrlSetOnEvent(-1, "Help9")


  $BtnNoPowerShellExecution = GUICtrlCreateButton("No PowerShell Exec.", 370+$DeltaX, $DeltaY+109, 140, 19)
  ...
  $BtnDocumentsAntiExploit = GUICtrlCreateButton("Documents Anti-Exploit", 370+$DeltaX, $DeltaY+128, 140, 19)
  GUICtrlSetOnEvent(-1, "DocumentsAntiExploit")
  $BtnHelpDocumentsAntiExploit = GUICtrlCreateButton("Help", 320+$DeltaX, $DeltaY+128, 40, 19)
  GUICtrlSetOnEvent(-1, "HelpDocumentsAntiExploit")

  $BtnDisableWSH = GUICtrlCreateButton("Disable Win. Script Host", 370+$DeltaX, $DeltaY+147, 140, 19)
  ...
  $BtnConfigureDefender = GUICtrlCreateButton("ConfigureDefender", 270+$DeltaX-6, $DeltaY+277, 90+12, 25)
  GUICtrlSetOnEvent(-1,"ConfigureDefender")
  GUICtrlSetBkColor(-1, 0xa0a0ff)
;  GUICtrlSetBkColor(-1, 0xeeeeee)

  $BtnGuiSkin = GUICtrlCreateButton("GUI Skin", 20+$DeltaX, $DeltaY+317, 80, 25)
...
;  _GUICtrlButton_Enable($BtnDefenderAntiPUA, True)
  _GUICtrlButton_Enable($BtnDocumentsAntiExploit, True)
...
$DocumentsAntiExploit = CheckDocumentsAntiExploit()
...
GUICtrlCreateListViewItem($DocumentsAntiExploit, $listview1)
...
Func Help0()
   ...
   Local $x = _ExtMsgBox(0,"&OK|NEXT|DOCUMENTATION", "", $help)
   If $x = 1 Then Return
   If $x = 2 Then Help0_1()
...
Func Help0_1()
   ...
   _ExtMsgBoxSet(1+4+8+32, 0, -1, -1, 10, "Arial", @DesktopWidth*0.5)
   Local $x = _ExtMsgBox(0,"&OK|NEXT|BACK", "", $help)
   If $x = 1 Then Return
   If $x = 2 Then Help0_2()
   If $x = 3 Then Help0()
EndFunc
...
Func Help0_2()
   ...
   Local $x = _ExtMsgBox(0,"&OK|BACK", "", $help)
   If $x = 1 Then Return
   If $x = 2 Then Help0_1()
EndFunc
...
Func Help1()
   ...
   _ExtMsgBoxSet(1+4+8+32, 0, -1, -1, 10, "Arial", @DesktopWidth*0.5)
   Local $x = _ExtMsgBox(0,"&OK|NEXT", "", $help)
   If $x = 1 Then Return
...
Func HelpDocumentsAntiExploit()
   Local $help = FileRead(@WindowsDir & "\Hard_Configurator\HELP\HelpDocumentsAntiExploit.txt")
;   MsgBox(262144,"", $help)
   _ExtMsgBoxSet(1+4+8+32, 0, -1, -1, 10, "Arial", @DesktopWidth*0.5)
   _ExtMsgBox(0,"OK", "", $help)
EndFunc
...
select
    case $BlockRemoteAccess = "ON"
           ...
           RegWrite($keyRemoteShell, $valRemoteShell,'REG_DWORD',Number('1'))
...
If $GreyDocumentsAntiExploit = 0 Then
   $reg = RegRead($BackupSwitchRestrictions, 'DocumentsAntiExploit')
   If @error = 0 Then
      If not ($reg = CheckDocumentsAntiExploit()) Then DocumentsAntiExploit1($reg)
   EndIf
EndIf
...
If $GreyDocumentsAntiExploit = 0 Then
    $reg = CheckDocumentsAntiExploit()
    If ($reg = 'ON' Or $reg = 'OFF') Then
       RegWrite($BackupSwitchRestrictions, 'DocumentsAntiExploit','REG_SZ', $reg)
       If $reg = 'ON' Then DocumentsAntiExploit1("OFF")
    EndIf
EndIf
...
; Add Protect Windows Folder and Protect Shortcuts.
  ...
  $DenyShortcuts = "OFF"
  Deny_Shortcuts('1')
$value = StringLower(RegRead('HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run', 'OneDrive'))
If StringInStr($value, StringLower('Microsoft\OneDrive\OneDrive.exe')) > 0 Then AddOneDrive1()
...
;  Delete Switch Restrictions backup
...
If (@OSVersion="WIN_10" or  @OSVersion="WIN_81" or @OSVersion="WIN_8" or @OSVersion="WIN_7") Then
   SRPExecutableTypes("light")
Else
   SRPExecutableTypes("light_and_powershell")
EndIf
...
If $isSRPinstalled = "Installed" Then
;   Opt("GUIOnEventMode", 1)
   $ARlistGUI = GUICreate("SRP Extensions", 270, 500, $X_SRPExtensionsGUI, $Y_SRPExtensionsGUI, -1)
   ...
   $BtnAddExtension = GUICtrlCreateButton("Add", 150, 15, 80, 30)
   GUICtrlSetOnEvent(-1, "AddItem")
   $BtnRemoveExtension = GUICtrlCreateButton("Remove", 150, 55, 80, 30)
   GUICtrlSetOnEvent(-1, "RemoveItem")
   GUICtrlCreateLabel ( "Windows Script Host", 135, 130, 120, 20, -1, -1)
   $BtnAddWindowsScriptHostExtensions = GUICtrlCreateButton("Add", 125, 150, 60, 20)
   GUICtrlSetOnEvent(-1, "AddWindowsScriptHostExtensions")
   $BtnRemoveWindowsScriptHostExtensions = GUICtrlCreateButton("Remove", 190, 150, 60, 20)
   GUICtrlSetOnEvent(-1, "RemoveWindowsScriptHostExtensions")
   GUICtrlCreateLabel ( "PowerShell", 160, 180, 120, 20, -1, -1)
   $BtnAddPowerShellExtensions = GUICtrlCreateButton("Add", 125, 200, 60, 20)
   GUICtrlSetOnEvent(-1, "AddPowerShellExtensions")
   $BtnRemovePowerShellExtensions = GUICtrlCreateButton("Remove", 190, 200, 60, 20)
   GUICtrlSetOnEvent(-1, "RemovePowerShellExtensions")
   $BtnSaveSRPExtensions = GUICtrlCreateButton("Save Extensions", 140, 260, 100, 30)
   GUICtrlSetOnEvent(-1, "SaveSRPExtensions")
   $BtnRestoreSRPExtensions = GUICtrlCreateButton("Restore Saved", 140, 300, 100, 30)
   GUICtrlSetOnEvent(-1, "RestoreSRPExtensions")
   $BtnRestoreDefaultExtensions = GUICtrlCreateButton("Restore Defaults", 140, 340, 100, 30)
   GUICtrlSetOnEvent(-1, "RestoreDefaultExtensions")
   $BtnEndExtensions = GUICtrlCreateButton("Close", 150, 430, 80, 30)
...
;     MsgBox(0,"", _StringToHex($SRPDefaultLevel) & @CRLF & $SRPDefaultLevel )
      ...
      Switch $SRPDefaultLevel
         ...
         case Else
           RegWrite('HKLM\SOFTWARE\Policies\Microsoft\Windows\Safer\CodeIdentifiers','DefaultLevel','REG_DWORD',Number('131072'))
           MsgBox(262144,"", "Wrong parameter. 'SRP Default Level' will be set to 'Basic User'.")
           $SRPDefaultLevel = 'Basic User'
      EndSwitch
   ...
   If $isSRPinstalled = "Installed" Then
      If $GreyWritableSubWindows = 0 Then
         ...
         Switch $WritableSubWindows
            ...
            case Else
               $WritableSubWindows = 'OFF'
               WritableSubWindows('1')
               MsgBox(262144,"", "Wrong parameter. 'Writable Win. SubFolders' will be set to 'ON'.")
         EndSwitch
      EndIf
      ...
      Switch $DenyShortcuts
         ...
         case Else
            $DenyShortcuts = "ON"
            Deny_Shortcuts('1')
         MsgBox(262144,"", "Wrong parameter. 'Deny Shortcuts' will be set to 'OFF'.")
      EndSwitch
   EndIf
...
;           Set ON123 and change to OFF
   ...
   If $GreyDocumentsAntiExploit = 0 Then
      $DocumentsAntiExploit =  RegReadLine('text_ini',22)
      $canread = $canread + Abs(@error)
      Switch $DocumentsAntiExploit
      case "ON"
         DocumentsAntiExploit1("ON")
      case "Partial"
         DocumentsAntiExploit1("Partial")
      case "OFF"
         DocumentsAntiExploit1("OFF")
      case Else
         DocumentsAntiExploit1("OFF")
        MsgBox(262144,"", "Wrong parameter. 'Documents Anti-Exploit' will be set to 'OFF'.")
      EndSwitch
   EndIf
...
$text = $text & $DocumentsAntiExploit & @CRLF
$text = $text & $SRPAllowExe & @CRLF
$text = $text & "END"
If $DocumentsAntiExploit = 'Partial' Then MsgBox(262144,"","<Documents Anti-Exe> option is set to 'Partial'. This setting will be ignored when downloading later this profile, so MS Office and Adobe Acrobat Reader settings will not be restored.")
...
Func _About()
    MsgBox(262144,"Hard_Configurator", "Windows Hardening Configurator" & @CRLF & "ver. " & $Hard_ConfiguratorVersion & @CRLF & "Copyright *  Andy Ful, June 2018")
EndFunc
...
Func CheckDocumentsAntiExploit()
   Local $n = 0
  ;Disabling DDE in Word 2010+, introduced via Windows Update KB4011575 in january 2018
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Word\Security', 'AllowDDE', Number('0'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Word\Security', 'AllowDDE', Number('0'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\Word\Security', 'AllowDDE', Number('0'))

  ;Disabling auto-update for any linked fields, including DDE, OLE. Links may be updated manually by the user
  ;Excel 2007+
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\12.0\Excel\Security', 'WorkbookLinkWarnings', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Excel\Security', 'WorkbookLinkWarnings', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'DDEAllowed', Number('0'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'DDECleaned', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'DontUpdateLinks', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'Options', Number('279'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Excel\Security', 'WorkbookLinkWarnings', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'DDEAllowed', Number('0'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'DDECleaned', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'DontUpdateLinks', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'Options', Number('279'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\Excel\Security', 'WorkbookLinkWarnings', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\Excel\Options', 'DDEAllowed', Number('0'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\Excel\Options', 'DDECleaned', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\Excel\Options', 'DontUpdateLinks', Number('1'))

  ;Outlook 2010+
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Word\Options\WordMail', 'DontUpdateLinks', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Word\Options\WordMail', 'DontUpdateLinks', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\Word\Options\WordMail', 'DontUpdateLinks', Number('1'))

  ;Word and Outlook 2007
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\12.0\Word\Options\vpref', 'fNoCalclinksOnopen_90_1', Number('1'))

  ;Word 2010+
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Word\Options', 'DontUpdateLinks', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Word\Options', 'DontUpdateLinks', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\Word\Options', 'DontUpdateLinks', Number('1'))
  ;One Note
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\OneNote\Options', 'DisableEmbeddedFiles', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\OneNote\Options', 'DisableEmbeddedFiles', Number('1'))

  ;MACROS
  ;Block macros in Microsoft Excel, Microsoft FrontPage, Microsoft Outlook, Microsoft PowerPoint, Microsoft Publisher, and Microsoft Word
  ...
  ;Block ActiveX in Office 2007+ (can be run only from secure locations)
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\Common\Security', 'DisableAllActiveX', Number('1'))

  ;OLE
  ;Block OLE in Office 2007+
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\12.0\Word\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\12.0\Excel\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\12.0\PowerPoint\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Word\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\Excel\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\14.0\PowerPoint\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Word\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\Excel\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\15.0\PowerPoint\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\Word\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\Excel\Security', 'PackagerPrompt', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Microsoft\Office\16.0\PowerPoint\Security', 'PackagerPrompt', Number('2'))

  ;ADOBE READEER
  ;Adobe Acrobat 10+ on Windows 64-bit Enables Protected Mode globally and thereby sandboxes Reader processes
  ;Adobe Reader 10+
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bProtectedMode', Number('1'))
  ;Adobe Reader 9.5 and 10.1.2+ 
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bDisableJavaScript', Number('1'))
  ;Enhanced security when the application is running in the browser Adobe Reader 9+
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bEnhancedSecurityInBrowser', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bEnhancedSecurityStandalone', Number('1'))
  ;11.0+ (Acrobat Reader)
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'iProtectedView', Number('2'))
  ;Enable AppContainer Adobe Reader beta spring 2018
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bEnableProtectedModeAppContainer', Number('1'))
  ;Adobe Acrobat 10+ on Windows 32-bit Enables Protected Mode globally and thereby sandboxes Reader processes
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bProtectedMode', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bDisableJavaScript', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bEnhancedSecurityInBrowser', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bEnhancedSecurityStandalone', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'iProtectedView', Number('2'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown', 'bEnableProtectedModeAppContainer', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'bProtectedMode', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'bDisableJavaScript', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'bEnhancedSecurityInBrowser', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'bEnhancedSecurityStandalone', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Wow6432Node\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'iProtectedView', Number('2'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'bProtectedMode', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'bDisableJavaScript', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'bEnhancedSecurityInBrowser', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'bEnhancedSecurityStandalone', Number('1'))
   $n = $n + _RegValueEqualTo('HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\11.0\FeatureLockDown', 'iProtectedView', Number('2'))

  ;Block JavaScript in Adobe Reader DC and Adobe Reader XI
  ;Block scripts
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\JSPrefs', 'bEnableJS', Number('0'))
  ;Script in one sandbox cannot access a script object in another sandbox
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\JSPrefs', 'bEnableGlobalSecurity', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\JSPrefs', 'bEnableJS', Number('0'))
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\JSPrefs', 'bEnableGlobalSecurity', Number('1'))

  ;Adobe Acrobat 10+ Enables Protected Mode and thereby sandboxes Reader processes
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Privileged', 'bProtectedMode', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Privileged', 'bProtectedMode', Number('1'))
  ;Turns on Protected View
   $n = $n + _RegValueEqualTo('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'iProtectedView', Number('2'))
  ;Enhanced security when the application is running in the browser (Adobe Reader 9+)
   $n = $n + _RegValueEqualTo('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'bEnhancedSecurityInBrowser', Number('1'))
  ;Enhanced security for the standalone application
   $n = $n + _RegValueEqualTo('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'bEnhancedSecurityStandalone', Number('1'))
  ;Enable Protected View for Outlook attachments Adobe Reader 10.1+
   $n = $n + _RegValueEqualTo('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'bEnableAlwaysOutlookAttachmentProtectedView', Number('0'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'iProtectedView', Number('2'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'bEnhancedSecurityInBrowser', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'bEnhancedSecurityStandalone', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'bEnableAlwaysOutlookAttachmentProtectedView', Number('0'))
  ...
  ;Disables opening non-PDF attachments in their native application.
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Originals', 'bAllowOpenFile', Number('0'))
  ;Disables opening attachments which are not PDF
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Originals', 'bSecureOpenFile', Number('1'))
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Originals', 'bAllowOpenFile', Number('0'))
   $n = $n + _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Originals', 'bSecureOpenFile', Number('1'))
   If $n = 86 Then Return 'ON'
   If _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Privileged', 'bProtectedMode', Number('1')) =1 Then $n= $n-1
   If _RegValueEqualTo('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Privileged', 'bProtectedMode', Number('1')) =1 Then $n= $n-1
   If ($n > 0  And $n < 84) Then Return 'Partial'
   If $n = 0  Then Return 'OFF'
EndFunc

Func DocumentsAntiExploit()
If $DocumentsAntiExploit = 'ON' Then DocumentsAntiExploit1('OFF')
If $DocumentsAntiExploit = 'Partial' Then DocumentsAntiExploit1('OFF')
If $DocumentsAntiExploit = 'OFF' Then DocumentsAntiExploit1('ON')
ShowRegistryTweaks()
EndFunc

Func DocumentsAntiExploit1($DAEflag)
...
If $DAEflag = "ON" Then
  ;Disabling DDE in Word 2010+, introduced via Windows Update KB4011575 in january 2018
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Word\Security', 'AllowDDE','REG_DWORD',Number('0'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Word\Security', 'AllowDDE','REG_DWORD',Number('0'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\Word\Security', 'AllowDDE','REG_DWORD',Number('0'))

  ;Disabling auto-update for any linked fields, including DDE, OLE. Links may be updated manually by the user
  ;Excel 2007+
   RegWrite('HKCU\Software\Microsoft\Office\12.0\Excel\Security', 'WorkbookLinkWarnings','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Excel\Security', 'WorkbookLinkWarnings','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'DDEAllowed','REG_DWORD',Number('0'))
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'DDECleaned','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'DontUpdateLinks','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'Options','REG_DWORD',Number('279'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Excel\Security', 'WorkbookLinkWarnings','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'DDEAllowed','REG_DWORD',Number('0'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'DDECleaned','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'DontUpdateLinks','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'Options','REG_DWORD',Number('279'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\Excel\Security', 'WorkbookLinkWarnings','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\Excel\Options', 'DDEAllowed','REG_DWORD',Number('0'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\Excel\Options', 'DDECleaned','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\Excel\Options', 'DontUpdateLinks','REG_DWORD',Number('1'))

  ;Outlook 2010+
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Word\Options\WordMail', 'DontUpdateLinks','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Word\Options\WordMail', 'DontUpdateLinks','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\Word\Options\WordMail', 'DontUpdateLinks','REG_DWORD',Number('1'))

  ;Word and Outlook 2007
   RegWrite('HKCU\Software\Microsoft\Office\12.0\Word\Options\vpref', 'fNoCalclinksOnopen_90_1','REG_DWORD',Number('1'))

  ;Word 2010+
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Word\Options', 'DontUpdateLinks','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Word\Options', 'DontUpdateLinks','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\Word\Options', 'DontUpdateLinks','REG_DWORD',Number('1'))
  ;One Note
   RegWrite('HKCU\Software\Microsoft\Office\16.0\OneNote\Options', 'DisableEmbeddedFiles','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\OneNote\Options', 'DisableEmbeddedFiles','REG_DWORD',Number('1'))
  ...
  ;ActiveX
  ;Block ActiveX in Office 2007+ (can be run only from secure locations)
   RegWrite('HKCU\Software\Microsoft\Office\Common\Security', 'DisableAllActiveX','REG_DWORD',Number('1'))

  ;OLE
  ;Block OLE in Office 2007+
   RegWrite('HKCU\Software\Microsoft\Office\12.0\Word\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\12.0\Excel\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\12.0\PowerPoint\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Word\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\14.0\Excel\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\14.0\PowerPoint\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Word\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\Excel\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\15.0\PowerPoint\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\Word\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\Excel\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Microsoft\Office\16.0\PowerPoint\Security', 'PackagerPrompt','REG_DWORD',Number('2'))
  ...
  ;Block JavaScript in Adobe Reader DC and Adobe Reader XI
  ;Block scripts
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\JSPrefs', 'bEnableJS','REG_DWORD',Number('0'))
  ;Script in one sandbox cannot access a script object in another sandbox
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\JSPrefs', 'bEnableGlobalSecurity','REG_DWORD',Number('1'))
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\JSPrefs', 'bEnableJS','REG_DWORD',Number('0'))
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\JSPrefs', 'bEnableGlobalSecurity','REG_DWORD',Number('1'))

  ;Adobe Acrobat 10+ Enables Protected Mode and thereby sandboxes Reader processes
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Privileged', 'bProtectedMode','REG_DWORD',Number('1'))
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Privileged', 'bProtectedMode','REG_DWORD',Number('1'))
  ;Turns on Protected View
   RegWrite('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'iProtectedView','REG_DWORD',Number('2'))
  ;Enhanced security when the application is running in the browser (Adobe Reader 9+)
   RegWrite('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'bEnhancedSecurityInBrowser','REG_DWORD',Number('1'))
  ;Enhanced security for the standalone application
   RegWrite('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'bEnhancedSecurityStandalone','REG_DWORD',Number('1'))
  ;Enable Protected View for Outlook attachments Adobe Reader 10.1+
   RegWrite('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'bEnableAlwaysOutlookAttachmentProtectedView','REG_DWORD',Number('0'))
   RegWrite('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'iProtectedView','REG_DWORD',Number('2'))
   RegWrite('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'bEnhancedSecurityInBrowser','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'bEnhancedSecurityStandalone','REG_DWORD',Number('1'))
   RegWrite('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'bEnableAlwaysOutlookAttachmentProtectedView','REG_DWORD',Number('0'))

  ;Block non-PDF attachments in their native (external) application
  ;Disables opening non-PDF attachments in their native application.
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Originals', 'bAllowOpenFile','REG_DWORD',Number('0'))
  ;Disables opening attachments which are not PDF
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Originals', 'bSecureOpenFile','REG_DWORD',Number('1'))
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Originals', 'bAllowOpenFile','REG_DWORD',Number('0'))
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Originals', 'bSecureOpenFile','REG_DWORD',Number('1'))
Else
  ;Word 2010+, introduced via Windows Update KB4011575 in january 2018
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Word\Security', 'AllowDDE')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Word\Security', 'AllowDDE')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\Word\Security', 'AllowDDE')
  ;Excel 2007+
   RegDelete('HKCU\Software\Microsoft\Office\12.0\Excel\Security', 'WorkbookLinkWarnings')
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Excel\Security', 'WorkbookLinkWarnings')
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'DDEAllowed')
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'DDECleaned')
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'DontUpdateLinks')
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Excel\Options', 'Options')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Excel\Security', 'WorkbookLinkWarnings')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'DDEAllowed')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'DDECleaned')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'DontUpdateLinks')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Excel\Options', 'Options')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\Excel\Security', 'WorkbookLinkWarnings')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\Excel\Options', 'DDEAllowed')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\Excel\Options', 'DDECleaned')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\Excel\Options', 'DontUpdateLinks')
CheckDocumentsAntiExploit()
  ;Outlook 2010+
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Word\Options\WordMail', 'DontUpdateLinks')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Word\Options\WordMail', 'DontUpdateLinks')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\Word\Options\WordMail', 'DontUpdateLinks')
  ;Word and Outlook 2007
   RegDelete('HKCU\Software\Microsoft\Office\12.0\Word\Options\vpref', 'fNoCalclinksOnopen_90_1')
  ;Word 2010+
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Word\Options', 'DontUpdateLinks')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Word\Options', 'DontUpdateLinks')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\Word\Options', 'DontUpdateLinks')
  ;One Note
   RegDelete('HKCU\Software\Microsoft\Office\16.0\OneNote\Options', 'DisableEmbeddedFiles')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\OneNote\Options', 'DisableEmbeddedFiles')
  ;MACROS
  ...
  ;ActiveX in Office 2007+ (can be run only from secure locations)
   RegDelete('HKCU\Software\Microsoft\Office\Common\Security', 'DisableAllActiveX')

  ;OLE
  ;OLE in Office 2007+
   RegDelete('HKCU\Software\Microsoft\Office\12.0\Word\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\12.0\Excel\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\12.0\PowerPoint\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Word\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\14.0\Excel\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\14.0\PowerPoint\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Word\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\Excel\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\15.0\PowerPoint\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\Word\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\Excel\Security', 'PackagerPrompt')
   RegDelete('HKCU\Software\Microsoft\Office\16.0\PowerPoint\Security', 'PackagerPrompt')
  ...
  ;JavaScript in Adobe Reader DC and Adobe Reader XI
   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\JSPrefs', 'bEnableJS')
  ;Script in one sandbox cannot access a script object in another sandbox
   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\JSPrefs', 'bEnableGlobalSecurity')
   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\JSPrefs', 'bEnableJS')
   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\JSPrefs', 'bEnableGlobalSecurity')
...
;   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Privileged', 'bProtectedMode')
;   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Privileged', 'bProtectedMode')
  ;Protected View
   RegDelete('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'iProtectedView')
  ;Enhanced security when the application is running in the browser (Adobe Reader 9+)
   RegDelete('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'bEnhancedSecurityInBrowser')
  ;Enhanced security for the standalone application
   RegDelete('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'bEnhancedSecurityStandalone')
  ;Protected View for Outlook attachments Adobe Reader 10.1+
   RegDelete('HKCU\Software\Adobe\Acrobat Reader\DC\TrustManager', 'bEnableAlwaysOutlookAttachmentProtectedView')
   RegDelete('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'iProtectedView')
   RegDelete('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'bEnhancedSecurityInBrowser')
   RegDelete('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'bEnhancedSecurityStandalone')
   RegDelete('HKCU\Software\Adobe\Acrobat Reader\11.0\TrustManager', 'bEnableAlwaysOutlookAttachmentProtectedView')
  ;non-PDF attachments in their native (external) application
  ;opening non-PDF attachments in their native application.
   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Originals', 'bAllowOpenFile')
  ;opening attachments which are not PDF
   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Originals', 'bSecureOpenFile')
   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Originals', 'bAllowOpenFile')
   RegDelete('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Originals', 'bSecureOpenFile')
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\DC\Privileged', 'bProtectedMode','REG_DWORD',Number('1'))
   RegWrite('HKCU\SOFTWARE\Adobe\Acrobat Reader\11.0\Privileged', 'bProtectedMode','REG_DWORD',Number('1'))
EndIf
