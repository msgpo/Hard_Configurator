#NoTrayIcon
Global Const $VERSION = "16.612.1119.0"
...
#AutoIt3Wrapper_Res_Fileversion=16.612.1119.0
#AutoIt3Wrapper_Res_Fileversion_AutoIncrement=p
#AutoIt3Wrapper_Res_Fileversion_First_Increment=y
#AutoIt3Wrapper_Res_ProductVersion=2.2
#AutoIt3Wrapper_Res_LegalCopyright=Copyright © 2015 Jos van der Zande
...
;~ #AutoIt3Wrapper_Run_After=Copy "%in%" "C:\Program Files (x86)\autoit3\SCAutoIt3Wrapper\SciTE\AutoIt3Wrapper.au3"
;~ #AutoIt3Wrapper_Run_After=Copy "directives.au3" "C:\Program Files (x86)\autoit3\SciTE\AutoIt3Wrapper\directives.au3"
...
;~ #Au3Stripper_Parameters=/mo /debug
...
Global $INP_Res_Language = "", $INP_Res_requestedExecutionLevel = "", $INP_RES_HiDpi = "", $INP_Res_Compatibility = "", $INP_Fileversion = "", $INP_Fileversion_New = "", $INP_Fileversion_AutoIncrement = "", $INP_Fileversion_First_Increment = "", $INP_LegalCopyright = ""
...
;~ 			Just for compatibility sake
		...
		Case $T_Var = "/tidy"
			$Option = "Tidy"
		Case $T_Var = "/RunAfter"
			; This option is a workaround for not compiling utilities anymore ans still wanting to copy the Script, updating version and running SVN commit.
...
SetDefaults($INP_RES_HiDpi, "n", "yes=y;no=n;1=y;0=n;4=n", "y;n", 0, 0)
...
If $Option = "Compile" Then
	...
	If $Pragma_Out <> "" Or $Pragma_Out_x64 <> "" Then
		If _VersionCompare($AUT2EXE_PGM_VER, "3.3.9.4") = 1 Then
			...
			__ConsoleWrite("-    Ignoring all #AutoIt3Wrapper_OutFile* and #AutoIt3Wrapper_Compile_both #directives!" & @CRLF)
		Else
			$Pragma_Out = ""
	...
	For $x = 1 To $INP_Run_Before[0]
		If StringStripWS($INP_Run_Before[$x], 3) <> "" Then
			...
			Else
				...
				If $INP_Run_Stop_OnError And $ExitCode Then
	...
	Switch $RType
		Case 1 ; *** RT_CURSOR
			...
			For $x = 1 To $CurCount
				...
				If $result[0] <> 1 Then
					__ConsoleWrite("Cursor UpdateResources: $result[0] = " & $result[0] & " - LastError:" & _WinAPI_GetLastError() & ":" & _WinAPI_GetLastErrorMessage())
				EndIf
			Next
			...
			If $result[0] <> 1 Then __ConsoleWrite("GroupCursor UpdateResources: $result[0] = " & $result[0] & " - LastError:" & _WinAPI_GetLastError() & ":" & _WinAPI_GetLastErrorMessage())
		Case 2 ; *** RT_BITMAP
			...
			If $bread > 0 Then
				...
				If $result[0] <> 1 Then __ConsoleWrite("UpdateResources other: $result[0] = " & $result[0] & " - LastError:" & _WinAPI_GetLastError() & ":" & _WinAPI_GetLastErrorMessage())
			EndIf
		Case 3 ; *** RT_ICON
			;ICO section
			$tSize = FileGetSize($InpResFile)
...
;~ 				If $IconHeight = 0 then $IconHeight = 256
				...
				If $result[0] <> 1 Then
					__ConsoleWrite("Icon UpdateResources: $result[0] = " & $result[0] & " - LastError:" & _WinAPI_GetLastError() & ":" & _WinAPI_GetLastErrorMessage())
				EndIf
			Next
			; Add Icongroup entry
			...
			If $result[0] <> 1 Then __ConsoleWrite("GroupIcon UpdateResources: $result[0] = " & $result[0] & " - LastError:" & _WinAPI_GetLastError() & ":" & _WinAPI_GetLastErrorMessage())
		Case 6 ; RT_STRING
			...
			For $i = 1 To $aLangs[0]
				...
				For $j = 1 To $aBlocks[0]
					...
					If $result[0] <> 1 Then __ConsoleWrite("String UpdateResources: $result[0] = " & $result[0] & " - LastError:" & _WinAPI_GetLastError() & ":" & _WinAPI_GetLastErrorMessage())
				Next
			Next
				...
				For $x = 1 To $Temp_Val[0]
					...
					Switch $Temp_Val[$x]
						...
						Case "Vista", "Windows7", "win7", "win8", "win81"
							;
						Case Else
							__ConsoleWrite("- Skipping #AutoIt3Wrapper_res_Compatibility directive invalid value:" & $Temp_Val & @CRLF)
					EndSwitch
				Next
				; ================ Other =========================================================================
...
;~ 	$INP_Allow_Decompile = GUICtrlRead($H_INP_NODECOMPILE)
	...
	If $Changes > 0 Then
		...
		Update_Directive($Full_source, $directives, "#AutoIt3Wrapper_Res_HiDpi", $INP_RES_HiDpi, "n", "1=y;0=n;4=n")
