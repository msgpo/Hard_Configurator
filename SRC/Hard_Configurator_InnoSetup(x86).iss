...
#define MyAppExeName "Hard_Configurator(x86).exe"
#define MySwitchName = "Switch Default Deny"
#define MySwitchExeName = "SwitchDefaultDeny(x86).exe"

[Setup]
; NOTE: The value of AppId uniquely identifies this application.
; Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
AppId={{F5B80342-A8D0-4A04-AC11-6F9C9968C3C5}
AppName={#MyAppName}
...
var
 ...
 begin
  ...
  if not Result then
  Order0 := 1;
  Result := not IsAppRunning('ConfigureDefender_x86.exe');
  if not Result then Order1 := 1;
  Result := not IsAppRunning('Hard_Configurator(x86).exe');
  if not Result then Order2 := 1;
  Result := not IsAppRunning('SwitchDefaultDeny(x86).exe');
  if not Result then Order3 := 1;
  Result := not IsAppRunning('DocumentsAntiExploit(x86).exe');
  ...
  If not (Order1 + Order2 + Order3 + Order4 = 0) Then begin
    MsgBox('Some processes related to Hard_Configurator are running on this or another account. They will be closed. This sometimes, can cause closing the installation process. If so then run again the installer.', mbError, MB_OK);
     ...
     If not (Order1 = 0) Then begin
        ShellExec('open',  'taskkill.exe', '/f /im ConfigureDefender_x86.exe','',SW_HIDE,ewNoWait,ErrorCode);
     end;
     If not (Order2 = 0) Then begin
        ShellExec('open',  'taskkill.exe', '/f /im Hard_Configurator(x86).exe','',SW_HIDE,ewNoWait,ErrorCode);
     end;
     If not (Order3 = 0) Then begin
        ShellExec('open',  'taskkill.exe', '/f /im SwitchDefaultDeny(x86).exe','',SW_HIDE,ewNoWait,ErrorCode);
     end;
     If not (Order3 = 0) Then begin
        ShellExec('open',  'taskkill.exe', '/f /im DocumentsAntiExploit(x86).exe','',SW_HIDE,ewNoWait,ErrorCode);
     end;
  end;
end;

[Files]
Source: "C:\Windows\Hard_Configurator\Hard_Configurator(x86).exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "C:\Windows\Hard_Configurator\RunAsSmartscreen(x86).exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "C:\Windows\Hard_Configurator\RunBySmartscreen(x86).exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "C:\Windows\Hard_Configurator\ConfigureDefender_x86.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "C:\Windows\Hard_Configurator\SwitchDefaultDeny(x86).exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "C:\Windows\Hard_Configurator\DocumentsAntiExploit(x86).exe"; DestDir: "{app}"; Flags: ignoreversion
...
Filename: "{app}\TOOLS\7zip(x86)\README.TXT"; Description: "View the 7-ZIP README file"; Flags: postinstall shellexec skipifsilent
Filename: "{app}\TOOLS\Autoruns(x86)\Eula.txt"; Description: "View the Autoruns EULA"; Flags: postinstall shellexec skipifsilent
Filename: "{app}\TOOLS\FullEventLogView(x86)\README.TXT"; Description: "View the FullEventView README file"; Flags: postinstall shellexec skipifsilent
