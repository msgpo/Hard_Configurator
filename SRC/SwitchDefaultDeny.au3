...
#AutoIt3Wrapper_Res_LegalCopyright=Copyright *  Andy Ful , June 2019
...
;					ConsoleWrite("Returned 1 = About button clicked." & @CRLF)
					...
					_Metro_MsgBox(0,"","SwitchDefaultDeny ver. 1.1.0.1" & @crlf & "Copyright by Andy Ful,   June 2019" & @crlf & "e-mail: Hard_Configurator@o2.pl" & @crlf & "dev. webpage: https://github.com/AndyFul/Hard_Configurator" ,500,12,"",20)
					_GUIDisable($Form1)
				Case "2"
...
;					ConsoleWrite("Returned 3 = Exit button clicked." & @CRLF)
		...
		Case $Toggle1
			If _Metro_ToggleIsChecked($Toggle1) Then
				...
				$TempRunAsSmartScreen = CheckRunAsSmartScreen()
				RegWrite('HKLM\SOFTWARE\Policies\Microsoft\Windows\safer_Hard_Configurator\Switch', 'RunAsSmartScreen', 'REG_SZ', $TempRunAsSmartScreen) 
			...
			Else
				...
				$TempRunAsSmartScreen = RegRead('HKLM\SOFTWARE\Policies\Microsoft\Windows\safer_Hard_Configurator\Switch', 'RunAsSmartScreen')
				...
				If ($TempDefaultDenyKey = 131072 or $TempDefaultDenyKey = 0) Then
					RegWrite($DefaultDenyKey, 'DefaultLevel', 'REG_DWORD', $TempDefaultDenyKey)
				        RunAsSmartScreen($TempRunAsSmartScreen)
					RegDelete('HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run', 'Hard_ConfiguratorSwitch')
...
Local $text1 = "The <Default Deny Protection> feature works only when Software Restrictions Policies are properly activated in Hard_Configurator. It can be useful for inexperienced users, when there are problems with application installations via Hard_Configurator 'Run As SmartScreen' or 'Run as administrator' features. In such case, the user can use SwitchDefaultDeny to:" & @crlf & "disable Default Deny Protection >> install applications via Run By SmartScreen >> enable Default Deny Protection." & @crlf &  @crlf & "When switching to the OFF setting, this utility simply changes SRP Default Security Level to Unrestricted, enables 'Run By SmartScreen' option in Explorer context menu, and adds the autorun entry to start with Windows." & @crlf &  @crlf & "When switching to the ON setting, it restores the old SRP Default Security Level setting ('Basic User' or 'Disallowed'), restores the old 'Run As SmartScreen' setting in Hard_Configurator, and removes the autorun entry."
Local $text2 = ""
Local $text3 = ""
_SetTheme("LightTeal")
_GUIDisable($Form1, 70)
...
Func CheckRunAsSmartScreen()
Local $keyname1 = "HKCR\*\shell\Run As SmartScreen\command"
Local $keyname2 = "HKCR\*\shell\Run By SmartScreen\command"
Local $valuename = ""
Local $_RunAsSmartScreen = RegRead ( $keyname1, $valuename )
Local $iskey = @error
RegRead ( $keyname2, $valuename )
$iskey = $iskey & @error
$_RunAsSmartScreen = $_RunAsSmartScreen & RegRead ( $keyname2, $valuename )
select
   case $_RunAsSmartScreen = @WindowsDir & '\Hard_Configurator\RunAsSmartscreen(x64).exe "%1" %*'
        $_RunAsSmartScreen = "Administrator"
   case $_RunAsSmartScreen = @WindowsDir & '\Hard_Configurator\RunAsSmartscreen(x86).exe "%1" %*'
        $_RunAsSmartScreen = "Administrator"
   case $_RunAsSmartScreen = @WindowsDir & '\Hard_Configurator\RunBySmartscreen(x64).exe "%1" %*'
        $_RunAsSmartScreen = "Standard User"
   case $_RunAsSmartScreen = @WindowsDir & '\Hard_Configurator\RunBySmartscreen(x86).exe "%1" %*'
        $_RunAsSmartScreen = "Standard User"
   case $iskey = "11"
        $_RunAsSmartScreen = "OFF"
   case Else
        ...
        $_RunAsSmartScreen = "OFF"
EndSelect
Return $_RunAsSmartScreen
EndFunc



Func RunAsSmartScreen($RASValue)
Local $keyname1 = 'HKCR\*\shell\Run As SmartScreen\command'
Local $keyname2 = 'HKCR\*\shell\Run By SmartScreen\command'
...
; Set "Administrator"
If $RASValue = "Administrator" Then
    If @OSArch="X64" Then
...
EndIf
; Set "Standard User"
If $RASValue = "Standard User" Then
    If @OSArch="X64" Then
